...
/* Alteem -For UART Read/Write Functionality */
#include "xmodem.h"
...
int _inbyte(unsigned short timeout) // msec timeout
{
  ...
  size_t bytesRead;
  /* Wait until there is data and Untill Time Out*/
...
int xmodemTransmitFilefrmSD(const char *fname, int srcsz)
{
  ...
  /* Open Provided File */
  File fcd;//  FILE *fcd;
  fcd = SD_MMC.open(fname);
  if (!fcd)
  {
    LOG_ERROR("Error: could not Open %s File for Xmodem Transmitt\r\n", fname);
    LOG_INTOSD("FileName : %s,Function : %s,LineNo. : %d - Error: could not Open %s File for Xmodem Transmitt\r\n", __FILE__, __FUNCTION__, __LINE__, fname);
    //fflush(fcd);
    fcd.close();
    return -4;
  }
...
start_trans:
      ...
      if (c > 0) {
        memset (&xbuff[3], 0, bufsz);
        // copy data to be txed into buffer 
        fcd.read(&xbuff[3], c);
        LOG_INFO("XModem Write Progress Bytes : %dK len:%d count:%d\r\n", (len / 1024), len, c);
#ifdef XMODEM_DEBUG
        int i = 0;
        for (i = 0; i < c; i++)
        {
